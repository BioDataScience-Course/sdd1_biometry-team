...
chart(data = biometry, ~ fastfood | imc_schedule ) +
  ...
  ylab("Densité")
```
>Lier avec le graphique qui suit? 
...
```
 ...
 Ce graphique nous permet de voir l'influence des fatsfood sur le poids. Dans un premier temps on remarque que chez les jeune (jusque 20-25 ans) le nombre de fastfood par semaine n'influence pas beaucoup leurs poids, on peut voir que entre 40 et 80kg chez les jeunes les fatsfood vont de 0 à 3 par semaine sans différence marquante dans leurs poid sauf une ou 2 exeptions. Chez les personnes plus agées (40 à 60ans et plus) même conclusion on ne voit pas différence significative au niveau des fastfood entre les différents poids, on voit que les personnes plus lourdes ne mange pas spécialement plus de fastfood et que les personnes plus mince n'en mange pas spécialement moins. 
...
On remarque que c'est plus au moins proprotionnel (sauf quelque exeption) la grande majorité à un poids qui est proportionnel à leurs taille ce qui montre que le hainaut n'est pas en obésit. De plus on observe que la plus part des femmes sont plus légère et plus petite que les hommes. 
...
```
 >Peut-être préférer ce type de graphique pour éviter la répétition des graphiques en camembert?
 
###Répartition des individus suivant leur IMC
```{r}
chart (biometry, ~ imc_schedule %fill=% imc_schedule )+
   geom_bar() + 
  coord_polar("y", start = 0) +
  theme_void() +
  scale_fill_viridis_d()
...
d <- chart(data = biometry, ~ acti_physique(0) %fill=% imc_schedule) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
  scale_fill_viridis_d()

e <- chart(data = biometry, ~ alcool(0) %fill=% imc_schedule) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
  scale_fill_viridis_d()

f <- chart(data = biometry, ~ fastfood(0) %fill=% imc_schedule) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0) +
  theme_void() +
  scale_fill_viridis_d()
...
combine_charts(list(d, e, f), common.legend = TRUE)
```
